...
import { galleryPreviews } from '../data/galleryImages';

const GalleryPage = () => {
    const galleryCategories = [
        { title: 'Bridal', slug: 'bridal', description: 'Graceful and elegant bridal gowns.', image: galleryPreviews.bridal },
        { title: 'Formal', slug: 'formal', description: 'Stunning dresses for any formal occasion.', image: galleryPreviews.formal },
        { title: 'Alterations', slug: 'alterations', description: 'See the before-and-after transformations.', image: galleryPreviews.alterations },
    ];
                        ...
                        <div key={cat.slug} className="gallery-category">
                            <Link to={`/gallery/${cat.slug}`} className="category-link">
                                <div className="category-image" style={{ aspectRatio: '4/3', overflow: 'hidden', backgroundColor: '#f0f0f0' }}>
                                    <img src={cat.image} alt={cat.title} loading="lazy" style={{ width: '100%', height: '100%', objectFit: 'cover' }}/>
                                </div>
